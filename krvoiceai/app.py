...
class EnlyAI:
    ...
    def _register_all_modules(self) -> None:
        ...
        # 创建唯一的 TTS 引擎实例（所有模块共享，避免重复加载模型浪费显存）
        tts_engine = TTSEngine(gpu_runner=gpu)
        ...
        self.modules: dict[str, BaseModule] = {
            ...
            "publish": Publisher(),
            # Podcast 复用同一个 TTS 引擎实例，避免重复加载 CosyVoice/Qwen3-TTS 模型
            "podcast": PodcastEngine(config=self.config, tts_engine=tts_engine, llm_client=llm),
        }
    ...
    def list_voices(self) -> list[dict]:
        ...
        # 3.5 CosyVoice 预置音色（7 个：anchor_female/anchor_male/narrator_*/english_*/seller_energetic）
        from .modules.tts_engine import COSYVOICE_PRESET_VOICES
        for vid, info in COSYVOICE_PRESET_VOICES.items():
            if vid in seen_ids:
                continue
            result.append({
                "voice_id": vid,
                ...
                "type": "preset",
                "provider": "cosyvoice",
                "supports_emotion": True,  # CosyVoice 支持 instruct 情绪控制
            })
            seen_ids.add(vid)
    ...
    def _pregenerate_voice_sample_async(self, voice_id: str) -> None:
        ...
        def _generate():
            try:
                ...
                engine = self.modules.get("tts")
                if engine is None:
                    engine = TTSEngine(gpu_runner=self.gpu, config=self.config)
                    engine.setup()
    ...
    def get_podcast_engine(self) -> PodcastEngine:
        """获取播客引擎实例（懒加载，复用共享 TTS 引擎）"""
        engine = self.modules.get("podcast")
        if engine is None:
            # 复用已注册的 TTS 引擎（避免重复加载模型）
            tts = self.modules.get("tts") or TTSEngine(gpu_runner=self.gpu, config=self.config)
    ...
    def health_check(self) -> dict:
        ...
        # 检查本地 TTS 服务（CosyVoice 等）— 优先于旧云端 GPU endpoint
        tts_server = self.config.get("tts.cosyvoice.server_url", "") or self.config.get("tts.api_base", "")
        local_tts_ok = False
        if tts_server:
            try:
                r = _httpx.get(f"{tts_server}/api/health", timeout=3)
                local_tts_ok = r.status_code == 200
            except Exception:
                local_tts_ok = False
        gpu_tts_ok = local_tts_ok or self.gpu.health_check_tts()

        # 检查本地数字人服务（LatentSync 等）
        avatar_server = self.config.get("avatar.latentsync.server_url", "") or self.config.get("avatar.musetalk.server_url", "")
        local_avatar_ok = False
        if avatar_server:
            try:
                r = _httpx.get(f"{avatar_server}/api/health", timeout=3)
                local_avatar_ok = r.status_code == 200
            except Exception:
                local_avatar_ok = False
        gpu_avatar_ok = local_avatar_ok or self.gpu.health_check_avatar()
