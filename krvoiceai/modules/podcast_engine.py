...
from .tts_engine import TTSEngine, QWEN3_PRESET_VOICES, COSYVOICE_PRESET_VOICES
...
# 口语润色模式：仅做措辞口语化 + 适度语气词，绝不扩充内容
PODCAST_REWRITE_SYSTEM = """你是一位口语化润色专家，任务是把文本转为口语化的多人对话播客剧本。

最高原则——只润色措辞，绝不扩充内容：
1. **标准普通话**：所有台词必须是标准、清晰的普通话。禁止使用方言词（如"咋""啥""嘞""俺""咱""中""得劲"等）、禁止吞音/连读的文字表示（如把"这样"写成"酱"、"你知道吗"写成"你知道嘛"）。发音必须能让 TTS 引擎正确朗读
2. **只做措辞口语化**：只把书面语措辞替换为口语化措辞——"综上所述"→"所以"、"具有重要意义"→"很重要"、"在...过程中"→"在...的时候"。不要重新组织段落结构，不要改变论述顺序
3. **适度语气词**：可以加入自然的语气词和叹词增强拟人感——"嗯""呢""啊""吧""哦""对""是的"。但要克制，每句最多1个语气词，不要每句都加，避免做作
4. **忠实保留全部内容**：必须覆盖原文的所有核心观点、事实信息、数据、案例，不得遗漏。但也不得添加原文没有的内容
5. **禁止扩充**：禁止添加原文没有的观点/故事/案例/比喻，禁止编造角色间的质疑或对立，禁止添加引申思考或开放性问题
6. **自然接话**：角色间可以有简短的自然接话（"对""是的""这个说得好"），但接话内容不得添加原文没有的信息
...
PODCAST_REWRITE_PROMPT = """请将以下内容润色为口语化的多人对话播客剧本。

重要：这是"口语润色"任务，不是"创作"任务。你只做两件事：①把书面语措辞换成口语措辞 ②适度加入语气词。绝对不要扩充内容、不要添加信息、不要增加原文没有的观点。
...
- 风格：{style}
- 台词行数由原文内容量决定，原文有多少内容就生成多少台词，绝对不要为了凑数而扩充
...
- 如果原文已经是剧本格式（角色名: 台词），只润色措辞，不改角色分配和台词顺序
- 主持人开场直接点明原文主题，不要编造故事或反差
- 结尾简短总结原文要点即可，不要编造原文未涉及的开放性问题
...
3. **适度扩展**：在原文观点基础上，可以补充相关的例子、类比、生活化比喻，让内容更易懂。但扩展总量不超过原文的 2 倍（原文 100 字则结果不超过 300 字），不要为每个观点都举例子，挑选 1-2 个关键点扩展即可
4. **增加互动**：角色间可以有自然的追问、质疑、补充，让对话更有化学反应
5. **口语化**：用"你想想看""说白了""我举个例"等自然口语过渡
6. **禁止跑题**：扩展内容必须与原文主题直接相关，禁止编造原文未提及的行业应用、故事场景或案例

""" + _SCRIPT_FORMAT_RULES

PODCAST_EXPAND_PROMPT = """请将以下内容扩展为多人对话播客剧本，在保留原意的基础上让内容更丰富饱满。
...
- {role_count} 个角色参与对话（{role_desc}）
- 风格：{style}
- 扩展后的总字数不超过原文的 2 倍，只挑选 1-2 个关键观点补充例子或类比，不要为每个观点都展开
- 角色名用简短好记的中文名
- 开头用注释行声明所有角色及性别
- 所有台词必须使用标准普通话
...
PODCAST_CONDENSE_PROMPT = """请将以下内容精简提炼为多人对话播客剧本，压缩冗余，保留核心观点。
...
class PodcastEngine(BaseModule):
    ...
    def tts(self) -> TTSEngine:
        """默认 TTS 引擎（向后兼容，返回共享的 TTS 引擎实例）"""
        if self._tts is None:
            # 不再创建新实例——使用全局默认 provider 的引擎
            # 这样所有 TTS 调用共享同一个模型加载，避免显存浪费
    ...
    def _detect_provider_by_voice(self, voice_id: str) -> str:
        ...
        - CosyVoice 预置音色（anchor_female/anchor_male/...）→ cosyvoice
        - 克隆音色（config/voices/ 下有目录）→ 默认 provider（cosyvoice/moss_nano 等）
        ...
        if voice_id in QWEN3_PRESET_VOICES:
            return "qwen3_tts"
        # CosyVoice 预置音色（7 个：anchor_female/anchor_male/narrator_*/english_*/seller_energetic）
        if voice_id in COSYVOICE_PRESET_VOICES:
            return "cosyvoice"
        # 克隆音色：按全局配置的默认 provider 处理（可能是 cosyvoice 或 moss_nano）
    ...
    def _get_tts_for_voice(self, voice_id: str) -> TTSEngine:
        ...
        对于 HTTP 类 provider（cosyvoice/edge_tts/mimo）只是路由切换，无模型加载开销；
        对于本地模型 provider（moss_nano/qwen3_tts）才需要独立实例缓存模型。
        """
        provider = self._detect_provider_by_voice(voice_id)

        # 如果检测到的 provider 与全局配置一致，直接复用共享引擎
        global_provider = self.config.get("tts.provider", "moss_nano")
        if provider == global_provider:
            return self.tts

        if provider not in self._tts_pool:
            # 只对不同 provider 创建新引擎实例（如 edge_tts 音色 vs cosyvoice 默认）
            import copy
