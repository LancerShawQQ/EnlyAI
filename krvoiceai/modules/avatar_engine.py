...
- musetalk:    本地 MuseTalk 服务（口型同步，独立 conda 环境，端口 8010）
- latentsync:  本地 LatentSync 服务（潜在扩散模型，独立 conda 环境，端口 8011）
...
class AvatarEngine(BaseModule):
    ...
    def __init__(self, config=None, gpu_runner: GPURunner | None = None,
        ...
        # LatentSync 配置（本地高质量引擎，潜在扩散模型，唇形质量超过 MuseTalk）
        # 字节跳动开源 Apache 2.0，8GB 显存可跑 256×256，需独立 conda 环境
        # Blackwell sm_120 兼容：PyTorch 2.7+cu128，fp16 推理
        self.latentsync_cfg = self.config.get("avatar.latentsync", {}) or {}
        self.latentsync_server_url = self.latentsync_cfg.get(
    ...
    def setup(self) -> None:
        ...
        elif self.provider == "latentsync":
            if not self.latentsync_server_url:
                self.logger.warning(
                    "LatentSync 引擎未配置 server_url，请在设置中心配置 LatentSync 服务地址"
                )
            else:
                self.logger.info(
                    f"数字人模块初始化 provider=latentsync "
                    f"server={self.latentsync_server_url} "
                    f"config={self.latentsync_config_name} "
                    f"steps={self.latentsync_inference_steps} "
                    f"res={self.latentsync_resolution}"
                )
    ...
    def run(self, ctx: JobContext) -> ModuleResult:
        ...
        try:
            ...
            if self.provider == "wav2lip":
                ...
                video_path = self._generate_latentsync(ctx, avatar_id, output_path)
    ...
    def _generate_cloud(
        ...
        """调用云端数字人 API（EchoMimic 等云端引擎，base64 payload 模式）"""
        self.logger.info(
    ...
    def _resume_cosyvoice(self) -> None:
        ...
        cosyvoice_url = self.config.get("tts.cosyvoice.server_url", "http://localhost:8012")
        try:
            self.logger.info("GPU 分时复用：恢复 CosyVoice 服务（重新加载模型）")
            r = httpx.post(f"{cosyvoice_url}/api/load", timeout=120)
            if r.status_code == 200:
                self.logger.info("CosyVoice 模型已重新加载")
            else:
                self.logger.warning(f"CosyVoice 恢复失败: HTTP {r.status_code}")
        except Exception as e:
            self.logger.warning(f"CosyVoice 恢复失败（下次 TTS 请求时会自动加载）: {e}")

    def _generate_latentsync(
        self, ctx: JobContext, avatar_id: str, output_path: Path
    ) -> Path:
        """使用 LatentSync 生成唇形同步视频（本地高质量引擎）

        LatentSync 是字节跳动开源的音频驱动潜在扩散模型（Apache 2.0），
        基于 Stable Diffusion + Whisper + U-Net，端到端生成唇形同步视频。
        与 MuseTalk/Wav2Lip 一样是"视频驱动"模式：保留原视频动作，只替换嘴型。

        优势（对比 MuseTalk）：
        - 潜在扩散模型，唇形质量超过 MuseTalk/Wav2Lip
        - TREPA 时序一致性，减少画面跳动
        - 1.5 优化中文音视频支持

        要求：需 NVIDIA GPU（8GB 显存可跑 256×256），独立 conda 环境。
        Blackwell sm_120 兼容：PyTorch 2.7+cu128，fp16 推理。
        部署：将 latentsync_server.py 部署到 LatentSync conda 环境中运行（端口 8011）。
        """
        import httpx
        ...
        # 获取参考人脸视频（与 Wav2Lip / MuseTalk 相同的逻辑）
        face_path = self._get_avatar_reference(avatar_id)
        if not face_path:
            raise RuntimeError(
                f"数字人 {avatar_id} 无参考照片/视频，请先上传真人照片或视频注册形象"
        ...
        self.logger.info(
            f"LatentSync 唇形同步 avatar={avatar_id} audio={ctx.audio_path.name} "
            f"duration={ctx.audio_duration:.1f}s face_ref={face_path.name} "
            f"config={self.latentsync_config_name} "
            f"steps={self.latentsync_inference_steps} "
        ...
        audio_path = ctx.audio_path
        if audio_path.suffix.lower() != ".wav":
            wav_path = ctx.work_dir / "latentsync_input.wav"
            self.ffmpeg.convert_audio(audio_path, wav_path)
            audio_path = wav_path

        # LatentSync 需要视频文件作为输入（视频驱动模式）
        # 如果参考是图片，先转为静态帧视频
        video_path = face_path
        if face_path.suffix.lower() in (".jpg", ".jpeg", ".png"):
        ...
        if self.latentsync_api_key:
            headers["Authorization"] = f"Bearer {self.latentsync_api_key}"

        # GPU 分时复用：8GB 显存无法同时跑 CosyVoice + LatentSync
        # 在调用 LatentSync 前，先暂停 CosyVoice 服务释放显存
        _cosyvoice_paused = self._suspend_cosyvoice_for_gpu()
        ...
        try:
            ...
            with httpx.Client(timeout=dynamic_timeout) as client:
                with client.stream(
                    ...
                    if resp.status_code != 200:
                        ...
                        raise RuntimeError(
                            f"LatentSync 生成失败 HTTP {resp.status_code}: {body[:500]}"
                        )
            ...
            elapsed = time.time() - start
            self.logger.info(
                f"LatentSync 数字人生成完成 size={len(video_bytes)} bytes 耗时={elapsed:.1f}s"
            )
    ...
    def register_avatar(
        ...
        if self.provider in ("wav2lip", "latentsync", "musetalk", "longcat", "echomimic"):
            # 本地引擎模式：直接保存参考素材（照片或视频）
            # LatentSync/MuseTalk/LongCat/Wav2Lip 都是"视频驱动"模式，
            # 需要原始参考视频保留动作，只替换嘴型。
            try:
                ...
                self.logger.info(
                    f"{self.provider} 形象注册成功: {avatar_id} -> {ref_path.name} ({kind})"
                )
                return True
            except Exception as e:
