...
function loadLLMSettings() {
  ...
  // API Key：已配置时输入框留空，通过占位符提示；未配置时也留空
  const apiKeyInput = document.getElementById('llm-api-key');
  apiKeyInput.value = '';
  if (llm.api_key_configured) {
    apiKeyInput.placeholder = '已配置（留空保持不变，输入新值则覆盖）';
    document.getElementById('llm-key-hint').textContent = '已配置';
    document.getElementById('llm-key-hint').className = 'hint-text hint-success';
  } else {
    apiKeyInput.placeholder = '请输入 API Key';
    document.getElementById('llm-key-hint').textContent = '未配置';
    document.getElementById('llm-key-hint').className = 'hint-text hint-warning';
  }
  // Base URL
...
async function saveLLMSettings() {
  ...
  // API Key：只在用户输入了新值时才发送，留空时不发送（后端保留原值）
  ...
  try {
    ...
    if (result.success) {
      ...
      _currentSettings = await api('/api/settings');
      loadLLMSettings();  // 重新加载，清空 API Key 输入框
      updateModelStatusBadges();
...
async function loadTTSSettings() {
  ...
  // CosyVoice3 音色回填
  const cosyVoice = (tts.cosyvoice && tts.cosyvoice.default_voice) || tts.default_voice || 'anchor_female';
  const cosySel = document.getElementById('tts-cosyvoice-voice');
  if (cosySel) cosySel.value = cosyVoice;
  // Qwen3-TTS 声音克隆配置回填
  ...
  const cosyPreviewBtn = document.getElementById('tts-cosyvoice-preview-btn');
  if (cosyPreviewBtn && !cosyPreviewBtn._bound) {
    cosyPreviewBtn._bound = true;
    cosyPreviewBtn.addEventListener('click', () => {
      const v = document.getElementById('tts-cosyvoice-voice').value;
      previewSettingsVoice(v, cosyPreviewBtn);
    });
  }
}
...
function onTTSProviderChange() {
  ...
  const cosyvoiceGroup = document.getElementById('tts-cosyvoice-voice-group');
  ...
  if (cosyvoiceGroup) cosyvoiceGroup.style.display = provider === 'cosyvoice' ? 'block' : 'none';
  ...
  if (provider === 'edge_tts') {
    ...
    const cosySel = document.getElementById('tts-cosyvoice-voice');
    defaultVoiceInput.value = cosySel ? cosySel.value : 'anchor_female';
  ...
  if (cosySel) {
    cosySel.onchange = () => {
      if (document.getElementById('tts-provider').value === 'cosyvoice') defaultVoiceInput.value = cosySel.value;
    };
  }
...
async function saveTTSSettings() {
  ...
  if (provider === 'edge_tts') {
    ...
    defaultVoice = document.getElementById('tts-cosyvoice-voice').value;
  }
  const data = {
    provider: provider,
  ...
  // CosyVoice3 额外保存 default_voice（预置音色 key）
  if (provider === 'cosyvoice') {
    data.cosyvoice = { default_voice: document.getElementById('tts-cosyvoice-voice').value };
  }
...
function loadAvatarSettings() {
  ...
  // LatentSync 配置回填
  const latentsync = avatar.latentsync || {};
  document.getElementById('latentsync-server-url').value = latentsync.server_url || '';
  document.getElementById('latentsync-api-key').value = latentsync.api_key || '';
  document.getElementById('latentsync-inference-steps').value = String(latentsync.inference_steps || 15);
  document.getElementById('latentsync-resolution').value = String(latentsync.resolution || 256);
  document.getElementById('latentsync-seed').value = latentsync.seed ?? -1;
  document.getElementById('latentsync-timeout').value = latentsync.timeout || 600;
}
...
function onAvatarProviderChange() {
  ...
  const latentsyncGroup = document.getElementById('avatar-latentsync-group');
  // api-base 仅对 echomimic 显示（latentsync/musetalk/longcat 有独立配置区）
  const showApiBase = provider === 'echomimic';
  ...
  // LatentSync 配置区仅对 latentsync 显示
  latentsyncGroup.style.display = provider === 'latentsync' ? 'block' : 'none';
  // wav2lip/mock 不需要 api-base
  // 自动填充默认地址（仅 echomimic 用通用 api-base）
...
async function saveAvatarSettings() {
  ...
  // LatentSync 配置
  data.latentsync = {
    server_url: document.getElementById('latentsync-server-url').value.trim(),
    api_key: document.getElementById('latentsync-api-key').value.trim(),
    inference_steps: parseInt(document.getElementById('latentsync-inference-steps').value) || 15,
    resolution: parseInt(document.getElementById('latentsync-resolution').value) || 256,
    seed: parseInt(document.getElementById('latentsync-seed').value),
    timeout: parseInt(document.getElementById('latentsync-timeout').value) || 600,
  };
  try {
